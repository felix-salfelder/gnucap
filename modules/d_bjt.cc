...
void MODEL_BUILT_IN_BJT::set_param_by_index(int i, std::string& value, int offset)
{
  ...
  default: throw Exception_Too_Many(unsigned(i), 49u, offset); break;
  }
}
/*--------------------------------------------------------------------------*/
  ...
  const TDP_BUILT_IN_BJT* t = &T;
	...
	trace4("!oik", q1, qb, dqbdve, dqbdvc);
        ...
        itested();
        {
          double dqbdq2;
          if(q2< 10) {
            qb=q1/2*(1+sqrt(1+log(1+exp(4*q2))));
            dqbdq2=exp(4*q2)/(sqrt(1+log(1+exp(4*q2)))*(1+exp(4*q2)));
          } else {
            qb=q1/2*(1+sqrt(1+4*q2));
            dqbdq2=1/sqrt(1+4*q2);
          }
          dqbdve = q1 * (qb * m->invearlyvoltr + c->oik  * gbe * dqbdq2);
          dqbdvc = q1 * (qb * m->invearlyvoltf + c->oikr * gbc * dqbdq2);
        }
#else
...
/*--------------------------------------------------------------------------*/
// FIXME: use map
void COMMON_BUILT_IN_BJT::set_param_by_name(string Name, string Value)
{
  if (Umatch (Name,"area"))       { area  = Value; }
  else if (Umatch (Name,"off"))   { off   = Value; }
  else if (Umatch (Name,"icvbe")) { icvbe = Value; }
  else if (Umatch (Name,"icvbe")) { icvce = Value; }
  else if (Umatch (Name,"temp"))  { temp_c= Value; }
  else {
  ...
  if (_sim->is_first_expand()) {
    ...
    if (!(_n[n_ib].n_())) {
      if (!OPT::rstray || (m->rb == 0. && m->rbm == 0.)) {
        ...
        _n[n_ib].new_model_node("ib", this);
      }
    }else{
      if (!OPT::rstray || (m->rb == 0. && m->rbm == 0.)) {
    ...
    if (!(_n[n_ie].n_())) {
      if (!OPT::rstray || m->re == 0.) {
        ...
        _n[n_ie].new_model_node("ie", this);
      }
    }else{
      if (!OPT::rstray || m->re == 0.) {
...
#ifdef LARS_BJT_HACK
    itested();
    vbe = pnj_limit((m->polarity * (_n[n_ib].v0()-_n[n_ie].v0())),
		    vbe, t->vt, t->Vcrit);
    vbc = pnj_limit((m->polarity * (_n[n_ib].v0()-_n[n_ic].v0())),
		    vbc, t->vt, t->Vcrit);
    vbx = m->polarity * (_n[n_b].v0()-_n[n_ic].v0());
    vcs = m->polarity * (_n[n_s].v0()-_n[n_ic].v0());
#else
