...
/*--------------------------------------------------------------------------*/
double SDP_BUILT_IN_MOS_BASE::dl_effdl_in(const COMMON_COMPONENT* cc) const
{
  assert(cc);
  const COMMON_BUILT_IN_MOS* c = prechecked_cast<const COMMON_BUILT_IN_MOS*>(cc);
  assert(c);
  ...
  double ret = (has_hard_value(c->l_in))
    ? m->lmlt : 1.;

  return ret;
}
/*--------------------------------------------------------------------------*/
double SDP_BUILT_IN_MOS_BASE::dw_effdw_in(const COMMON_COMPONENT* cc) const
{
  assert(cc);
  const COMMON_BUILT_IN_MOS* c = prechecked_cast<const COMMON_BUILT_IN_MOS*>(cc);
  assert(c);
  ...
  double ret = (has_hard_value(c->w_in))
    ? m->wmlt : 1.;
...
void MODEL_BUILT_IN_MOS_BASE::precalc_first()
{
        ...
        bti_model_name.value() == std::string("none") )
    {
      ...
      if ((!has_hard_value(rs)) && has_hard_value(rd)) {
	error(((!_sim->is_first_expand()) ? (bDEBUG) : (bWARNING)),
	      long_label() + ": rd input, but not rs. setting rs = 0.\n");
	rs.set_default(0.);
      } else if ((!has_hard_value(rd)) && has_hard_value(rs)) {untested();
	error(((!_sim->is_first_expand()) ? (bDEBUG) : (bWARNING)), 
...
void MODEL_BUILT_IN_MOS_BASE::set_param_by_index(int i, std::string& value, int offset)
{
  ...
  default: throw Exception_Too_Many(unsigned(i), 34u, offset); break;
  }
}
/*--------------------------------------------------------------------------*/
bool MODEL_BUILT_IN_MOS_BASE::param_is_printable(int i)const
{
  ...
  case 34:  return bti_model_name.has_hard_value();
...
/*--------------------------------------------------------------------------*/
double MODEL_BUILT_IN_MOS_BASE::dgmdl_in(const DEV_BUILT_IN_MOS* d)const
{
  ...
  return m->dgmdl_eff(d) * s->dl_effdl_in(c);
}
/*--------------------------------------------------------------------------*/
double MODEL_BUILT_IN_MOS_BASE::dgmdw_in(const DEV_BUILT_IN_MOS* d)const
{
  const COMMON_BUILT_IN_MOS* c = prechecked_cast<const COMMON_BUILT_IN_MOS*>(d->common());
  const MODEL_BUILT_IN_MOS_BASE* m = this;
  assert(m);
